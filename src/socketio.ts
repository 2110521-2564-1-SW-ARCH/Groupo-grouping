...
import {autogroupHandlerBuilder, boardHandlerBuilder, chatHandlerBuilder, groupHandlerBuilder, tagHandlerBuilder, transitHandlerBuilder} from "./socketio/handler";
...
io.on("connection", (socket: Socket<DefaultEventsMap, DefaultEventsMap>) => {
    ...
    // board event is an event for board CRUD
    socket.on("board", boardHandlerBuilder(ctx));

    // TODO: implement in chat service
    socket.on("chat", chatHandlerBuilder(ctx));

    // transit event is an event that indicate the group of the user is change
    socket.on("autogroup", autogroupHandlerBuilder(ctx));
